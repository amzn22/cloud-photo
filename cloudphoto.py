...
def upload_photos(path, album):
    s3.upload_file(path, 'cloud-photo', uuid.uuid4().__str__() + "_" + album + ".jpg")


def download_photos(path, album):
    names = []
    for key in s3.list_objects(Bucket='cloud-photo')['Contents']:
        names.append(key['Key'])
    for name in names:
        x = name.split("_")
        if album in x[1]:
            s3.download_file('cloud-photo', name, path + name)
